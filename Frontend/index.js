...
form.addEventListener("submit", function (event) {
  ...
  axios
    .post("http://localhost:5000/user/add-user", userDetails, {
...
async function loadPage(month, year, page) {
  ...
  try {
    const response = await axios.get(
      `http://localhost:5000/user/get-expenses/${month}/${year}/${rows}?page=${page}`,
      {
        headers: { Authorization: token },
      }
    );
...
async function handleDateChange() {
  ...
  try {
    const token = localStorage.getItem("token");
    const response = await axios.get(
      `http://localhost:5000/user/get-expense/${day}/${month}/${year}`,
      { headers: { Authorization: token } }
    );
...
async function isPremium() {
  const token = localStorage.getItem("token");
  const response = await axios.get(`http://localhost:5000/purchase/isPremium`, {
    headers: { Authorization: token },
  });
...
async function leaderboard() {
  ...
  if (decodedToken.isPremium == true) {
    const response = await axios.get(
      `http://localhost:5000/purchase/leaderboard`,
      { headers: { Authorization: token } }
    );
...
async function displayDownloads() {
  ...
  if (decodedToken.isPremium == true) {
    try {
      ...
      const response = await axios.get("http://localhost:5000/get-downloads", {
        headers: { Authorization: token },
      });
